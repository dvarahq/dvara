 ...
 *
 * <p>A pattern that tries every start position inside a long run, and walks to the end of the run from each
 * one, costs the square of the run.
 */
class LongRunScanTest {

    private static final int LENGTH = 64 * 1024;

    /** The email pattern without its anchor: the anchored one must find exactly what this finds. */
    private static final Pattern UNANCHORED_EMAIL =
    ...
    @Test
    void theEmailPatternFindsWhatTheUnanchoredFormFinds() {
        ...
        for (int i = 0; i < 20_000; i++) {
            ...
            assertThat(matches(current, s)).as("matches in \"%s\"", s).isEqualTo(matches(UNANCHORED_EMAIL, s));
        }
    }
    ...
    void twoAddressesJoinedByALocalPartCharacterAreBothFound() {
        ...
        assertThat(matches(current, s)).isEqualTo(matches(UNANCHORED_EMAIL, s)).hasSize(2);
    }
