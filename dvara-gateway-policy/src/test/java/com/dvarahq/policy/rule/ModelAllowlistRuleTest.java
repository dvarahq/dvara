...
class ModelAllowlistRuleTest {

    private final PolicyContext ctx = PolicyContext.empty();

    @Test
    void matches_modelNotInAllowlist_returnsTrue() {
        var rule = new ModelAllowlistRule(Set.of("gpt-4o", "claude-sonnet-4-5"));
        var request = ChatRequest.builder().model("gpt-3.5-turbo").build();
        assertThat(rule.matches(ctx, request)).isTrue();
    }

    @Test
    void doesNotMatch_modelInAllowlist_returnsFalse() {
        var rule = new ModelAllowlistRule(Set.of("gpt-4o", "claude-sonnet-4-5"));
    ...
    void nullModel_returnsFalse() {
        var rule = new ModelAllowlistRule(Set.of("gpt-4o"));
